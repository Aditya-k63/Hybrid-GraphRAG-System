...
def evaluate_batch(questions: list[str], answers: list[str], contexts_list: list[list[str]]) -> dict:
    ...
    if not settings.ENABLE_LIVE_LLM:
        results = [
            _fallback_evaluate(q, a, contexts)
            for q, a, contexts in zip(questions, answers, contexts_list)
        ]
        if not results:
            return {"error": "no evaluation inputs"}
        return {
            "count": len(results),
            ...
            "overall_score": round(sum(r["overall_score"] for r in results) / len(results), 4),
